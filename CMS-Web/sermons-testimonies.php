<?php
require_once 'includes/sermons-testimonies-handler.php';

// Prepare dynamic content for placeholders
$page_title = 'Sermons & Testimonies';

// Stats block
$stats = '<div class="row mb-4">'
...
$tabs = '<div class="custom-tabs">'
    .'<a href="?type=sermons'. (!empty($search) ? '&search='.urlencode($search) : '') . (!empty($category) ? '&category='.urlencode($category) : '') .'" class="custom-tab-btn '.($type==='sermons'?'active':'').'">'
    .'<i class="fas fa-church"></i> Sermons</a>'
    .'<a href="?type=testimonies'. (!empty($search) ? '&search='.urlencode($search) : '') . (!empty($category) ? '&category='.urlencode($category) : '') .'" class="custom-tab-btn '.($type==='testimonies'?'active':'').'">'
...
$search_form = '<form method="GET" class="search-bar">'
    ...
    .'<input type="text" name="search" class="search-input" placeholder="Search '.htmlspecialchars($type).' by title, author, or description..." value="'.htmlspecialchars($search).'">'
...
$filter_tabs = '<div class="filter-tabs">'
    .'<a href="?type='.$type.(!empty($search)?'&search='.urlencode($search):'').'" class="filter-btn '.(empty($category)?'active':'').'">All</a>';
foreach ($categories as $cat) {
    $filter_tabs .= '<a href="?type='.$type.'&category='.urlencode($cat['category']).(!empty($search)?'&search='.urlencode($search):'').'" class="filter-btn '.($category===$cat['category']?'active':'').'">'.ucfirst(htmlspecialchars($cat['category'])).'</a>';
}
$filter_tabs .= '</div>';
...
if (empty($content)) {
    $content_html = '<div class="no-results">'
        ...
        .'<p>No '.htmlspecialchars($type).' found matching your search criteria.</p>'
        .'<a href="?type='.$type.'" class="btn btn-custom-primary">View All '.ucfirst($type).'</a>'
        .'</div>';
} else {
    ...
    foreach ($content as $item) {
        ...
        $content_html .= '<div class="card-header"><div class="card-icon '.($item['type']==='sermon'?'sermon-icon':'testimony-icon').'">'
            .'<i class="'.SermonsTestimoniesHandler::getFileTypeIcon($item['file_type']).'"></i></div>';
        ...
        if (SermonsTestimoniesHandler::isVideo($item['file_type'])) {
            $content_html .= '<div class="media-container"><video class="media-preview" controls preload="metadata">'
                .'<source src="'.htmlspecialchars($item['file_path']).'" type="video/mp4">Your browser does not support the video tag.'</video>';
            $content_html .= '<div class="media-overlay"><button class="btn btn-play-overlay" onclick="playMedia(this)"><i class="fas fa-play"></i></button></div></div>';
        } elseif (SermonsTestimoniesHandler::isAudio($item['file_type'])) {
            $content_html .= '<div class="media-container"><audio class="media-preview" controls preload="metadata">'
                .'<source src="'.htmlspecialchars($item['file_path']).'" type="audio/mpeg">Your browser does not support the audio tag.'</audio>';
            $content_html .= '<div class="media-overlay"><button class="btn btn-play-overlay" onclick="playMedia(this)"><i class="fas fa-volume-up"></i></button></div></div>';
        }
        ...
        if (SermonsTestimoniesHandler::isVideo($item['file_type']) || SermonsTestimoniesHandler::isAudio($item['file_type'])) {
            $content_html .= '<button class="btn btn-watch-now" onclick="playMediaInCard(this, \\''.htmlspecialchars($item['file_path']).'\\', \\''.$item['file_type'].'\\', \\''.htmlspecialchars($item['title']).'\\')">'
                .'<i class="fas fa-'.(SermonsTestimoniesHandler::isVideo($item['file_type'])?'play':'volume-up').'"></i> '
                .(SermonsTestimoniesHandler::isVideo($item['file_type'])?'Watch Now':'Listen Now').'</button>';
        }
        $content_html .= '<button class="btn btn-like" onclick="likeVideo('.$item['id'].', this)"><i class="fas fa-heart"></i> <span class="like-count">'.(isset($item['likes'])?$item['likes']:0).'</span></button>';
        ...
        if ($item['file_size']) {
            $content_html .= ' • <i class="fas fa-file"></i> '.SermonsTestimoniesHandler::formatFileSize($item['file_size']);
        }
